...
#endif
          ,
          fp(0), timescale_unit(), timescale_set_by_user(false), filename_(), initialized_(false), trace_delta_cycles_(false)
    {
        if (!name || !*name)
        {
...
//
    ...
    sc_trace_file_base::initialize()
    {
        ...
        if (!tracing_initialized_)
        {
            ...
            {
                sc_report_handler::set_actions(SC_ID_TRACING_TIMESCALE_DEFAULT_, SC_INFO, SC_DO_NOTHING);
                sc_report_handler::set_actions(SC_ID_TRACING_VCD_DUPLICATE_TIME_, SC_WARNING, SC_DO_NOTHING);
            }
        }
        ...
        if (!timescale_set_by_user)
        {
            ...
            SC_REPORT_INFO(SC_ID_TRACING_TIMESCALE_DEFAULT_, ss.str().c_str());
        }
    ...
    sc_trace_file_base::set_time_unit(double v, sc_time_unit tu)
    {
        if (initialized_)
        {
            ...
            SC_REPORT_ERROR(SC_ID_TRACING_ALREADY_INITIALIZED_, ss.str().c_str());
            return;
        }
        ...
        default:
        {
            ...
            ss << "unknown time unit:" << tu
               << " (" << filename_ << ")";
            SC_REPORT_WARNING(SC_ID_TRACING_TIMESCALE_UNIT_, ss.str().c_str());
        }
        }
