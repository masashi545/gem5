...
namespace sc_core
{
    ...
    vcd_trace_file::vcd_trace_file(const char *name)
        : sc_trace_file_base(name, "vcd"), vcd_name_index(0), previous_time_units_low(0), previous_time_units_high(0), traces()
    {
    }
...
#undef DEFN_TRACE_METHOD_LONG_LONG
    ...
    vcd_trace_file::cycle(bool this_is_a_delta_cycle)
    {
        ...
        if ((now_units_low > previous_time_units_low && now_units_high == previous_time_units_high) || now_units_high > previous_time_units_high)
        {
        ...
        {
            if (now_units_low == previous_time_units_low && now_units_high == previous_time_units_high)
            {
        ...
        if (this_is_a_delta_cycle)
        {
            ...
            if (!warned)
            {
                SC_REPORT_INFO(SC_ID_TRACING_VCD_DELTA_CYCLE_, sc_time(timescale_unit, SC_SEC)
                                                                   .to_string()
                                                                   .c_str());
                warned = true;
            }
            ...
            (now_units_high != 0 || now_units_low != 0))
        {
            ...
            if (!warned)
            {
                ...
                SC_REPORT_WARNING(SC_ID_TRACING_VCD_DUPLICATE_TIME_, ss.str().c_str());
                // warned = true;
            }
        }
...
vcd_trace_file::create_vcd_name(std::string* p_destination)
{
    ...
    remove_vcd_name_problems(vcd_trace const *vcd, std::string &name)
    {
        ...
        if (braces_removed && !warned)
        {
            ...
            SC_REPORT_WARNING(SC_ID_TRACING_OBJECT_NAME_FILTERED_, ss.str().c_str());
        }
    }
