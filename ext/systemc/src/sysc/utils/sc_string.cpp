...
namespace sc_dt
{
    ...
    class sc_string_rep
    {
        ...
        sc_string_rep(int size = 16) : ref_count(1), alloc(sc_roundup(size, 16)), str(new char[alloc])
        {
            *str = '\0';
        }
    ...
    sc_string_rep::sc_string_rep(const char *s, int n) : ref_count(1), alloc(0), str(0)
    {
        if (s && n > 0)
        {
            alloc = 1 + n;
    ...
    sc_string_old::sc_string_old(const char *s, int n) : rep(new sc_string_rep(s, n))
    {
    }
...
#define sc_vsnprintf vsnprintf
    ...
    sc_string_old::fmt_length() const
    {
        ...
        if (rep->str[result] == '.') // precision
        {
            ...
            while (is_delimiter("0123456789*", result))
                result++;
            if (old_result == result) // error in format
                return 0;
        }
        if (is_delimiter("hlL", result))
            result++; // I64 is not supported
