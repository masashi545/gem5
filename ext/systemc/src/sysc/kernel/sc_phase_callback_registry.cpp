  ...
  , m_cb_eval_vec()
#endif
          ,
          m_cb_update_vec(), m_cb_timestep_vec()
    {
    }
    ...
    namespace /* anonymous */
    {
            ...
            : std::unary_function<sc_phase_callback_registry::entry, bool>
        {
            ...
            explicit entry_match(ref_type ref)
                : ref_(ref)
            {
            }
    ...
    sc_phase_callback_registry::validate_mask(cb_type &cb, mask_type m, bool warn = false)
    {
        if (SC_UNLIKELY_(m & ~SC_PHASE_CALLBACK_MASK))
        {
            if (warn)
            {
                ...
                ss << cb.name() << ": invalid phase callback mask: "
                   << (sc_status)m;
                SC_REPORT_WARNING(SC_ID_PHASE_CALLBACK_REGISTER_, ss.str().c_str());
            }
        ...
        check_mask = (SC_ELABORATION | SC_BEFORE_END_OF_ELABORATION | SC_END_OF_ELABORATION);
    ...
    warn_phase_callbacks(sc_core::sc_object const &obj)
    {
        ...
        if (!warned)
        {
            ...
            SC_REPORT_WARNING(SC_ID_PHASE_CALLBACKS_UNSUPPORTED_, ss.str().c_str());
        }
    }
