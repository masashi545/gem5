...
namespace sc_core
{
    ...
    sc_simcontext::init()
    {
        ...
        m_write_check = ((write_check == 0) || strcmp(write_check, "DISABLE")) ? true : false;
    ...
    sc_simcontext::sc_simcontext() : m_object_manager(0), m_module_registry(0), m_port_registry(0),
        ...
        sc_process_host *host_p, const sc_spawn_options *opt_p)
    {
        ...
        { // dynamic process
            if (!handle->dont_initialize())
            {
                ...
                if (SC_UNLIKELY_(m_simulation_status & (SC_END_OF_UPDATE | SC_BEFORE_TIMESTEP)))
                {
                    ...
                    msg << m_simulation_status
                        << ":\n\t immediate method spawning of "
                           "`"
                        << handle->name() << "' ignored";
                    SC_REPORT_WARNING(SC_ID_PHASE_CALLBACK_FORBIDDEN_, msg.str().c_str());
                }
                else
