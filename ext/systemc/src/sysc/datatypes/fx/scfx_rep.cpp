...
#undef SCFX_FAIL_IF_
    ...
    scfx_rep::to_double() const
    {
        ...
        if (exp < SCFX_IEEE_DOUBLE_E_MIN - static_cast<int>(SCFX_IEEE_DOUBLE_M_SIZE))
        {
            id = 0.;
            return id;
        }
        ...
        if (exp < SCFX_IEEE_DOUBLE_E_MIN)
        {
            ...
            {
                m1 = m1 >> subnormal_shift | m0 << (bits_in_word - subnormal_shift);
    ...
    print_dec(scfx_string &s, const scfx_rep &num, int w_prefix, sc_fmt fmt)
    {
        ...
        if (!int_part.is_zero())
        {
            double int_wl = (int_part.m_msw - int_part.m_wp) * bits_in_word + scfx_find_msb(int_part.m_mant[int_part.m_msw]) + 1;
        ...
        if (!frac_part.is_zero())
        {
            ...
            double frac_wl = (frac_part.m_wp - frac_part.m_msw) * bits_in_word - scfx_find_msb(frac_part.m_mant[frac_part.m_msw]) - 1;
                ...
                sc_fmt fmt, const scfx_params *params)
    {
        ...
        else
        {
            ...
            {
                msb = (b.m_msw - b.m_wp) * bits_in_word + scfx_find_msb(b.m_mant[b.m_msw]) + 1;
                ...
                lsb = (b.m_lsw - b.m_wp) * bits_in_word + scfx_find_lsb(b.m_mant[b.m_lsw]);
            }
        }
    ...
    add_scfx_rep(const scfx_rep &lhs, const scfx_rep &rhs, int max_wl)
    {
        ...
        if (lhs.is_nan() || rhs.is_nan() || (lhs.is_inf() && rhs.is_inf() && lhs.m_sign != rhs.m_sign))
        {
            result.set_nan();
            return &result;
        }
             ...
             int max_wl)
    {
        ...
        if (lhs.is_nan() || rhs.is_nan() || (lhs.is_inf() && rhs.is_zero()) || (lhs.is_zero() && rhs.is_inf()))
        {
            result.set_nan();
            return;
        }
    ...
    div_scfx_rep(const scfx_rep &lhs, const scfx_rep &rhs, int div_wl)
    {
        ...
        int msb_lhs = scfx_find_msb(lhs.m_mant[lhs.m_msw]) + (lhs.m_msw - lhs.m_wp) * bits_in_word;
        int msb_rhs = scfx_find_msb(rhs.m_mant[rhs.m_msw]) + (rhs.m_msw - rhs.m_wp) * bits_in_word;
