...
namespace sc_core
{
    ...
    sc_port_base::sc_port_base(
        int max_size_, sc_port_policy policy) : sc_object(sc_gen_unique_name("port")),
                                                m_bind_info(NULL)
    {
    ...
    sc_port_base::sc_port_base(
        const char *name_, int max_size_, sc_port_policy policy) : sc_object(name_),
                                                                   m_bind_info(NULL)
    {
        simcontext()->get_port_registry()->insert(this);
        m_bind_info = new sc_bind_info(max_size_, policy);
    ...
    sc_port_base::insert_parent(int i)
    {
        ...
        vec[i]->parent = 0;
        if (parent->m_bind_info->vec.size() == 0)
            return;
