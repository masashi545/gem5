...
#define convert(function)                  \
    ...
    {                                      \
    case 1:                                \
        function<dt<1>>(&txn, bus_width);  \
        break;                             \
    case 2:                                \
        function<dt<2>>(&txn, bus_width);  \
        break;                             \
    case 4:                                \
        function<dt<4>>(&txn, bus_width);  \
        break;                             \
    case 8:                                \
        function<dt<8>>(&txn, bus_width);  \
        break;                             \
    case 16:                               \
        function<dt<16>>(&txn, bus_width); \
        break;                             \
    case 32:                               \
        function<dt<32>>(&txn, bus_width); \
        break;                             \
    default:                               \
        cout << "bad data width\n";        \
        exit(1);                           \
...
void test_a_conversion(char cmd, tlm_generic_payload &txn, std::ifstream &fin)
{
    ...
    for (unsigned b = 0; b < txn.get_data_length(); b++)
    {
        char tmp;
        fin >> tmp;
        if ((tmp == '0') || (tmp == '1') || (tmp == 'x'))
            byte_enable_legible[b] = tmp;
        else
            break;
    }
    ...
    {
    case 0:
        convert(tlm_to_hostendian_generic);
        break;
    case 1:
        convert(tlm_to_hostendian_word);
        break;
    case 2:
        convert(tlm_to_hostendian_aligned);
        break;
    case 3:
        convert(tlm_to_hostendian_single);
        break;
    case 4:
        convert(local_single_tohe);
        break;
    default:
        cout << "no such converter as " << converter << endl;
        exit(1);
    }
...
#else
    cout << "  Txn data pointer = " << (txn.get_data_ptr() == initiator_mem + initiator_offset ? "unchanged" : "changed") << endl;
    if (txn.get_byte_enable_ptr() != 0)
    {
        cout << "  Byte enables and byte enable pointer = ";
        for (unsigned int i = 0; i < txn.get_data_length(); i++)
            cout << (txn.get_byte_enable_ptr()[i] ? '1' : '0');
        cout << ", " << (txn.get_byte_enable_ptr() == original_byte_enable ? "unchanged" : "changed") << endl;
    }
#endif
    cout << "  Byte enable length = " << txn.get_byte_enable_length() << endl;
    cout << "  Streaming width = " << txn.get_streaming_width() << endl;
    ...
    else
    {
        ...
        {
        case 0:
            convert(tlm_from_hostendian_generic);
            break;
        case 1:
            convert(tlm_from_hostendian_word);
            break;
        case 2:
            convert(tlm_from_hostendian_aligned);
            break;
        case 3:
            convert(tlm_from_hostendian_single);
            break;
        case 4:
            convert(local_single_fromhe);
            break;
        default:
            cout << "no such converter as " << converter << endl;
            exit(1);
    ...
    cout << "  target = " << target_mem << endl
         << endl;

    // clean up
    delete[] byte_enable_legible;
    if (original_byte_enable != 0)
        delete[] original_byte_enable;
}
