...
#if DEBUG
void Dump_R(char *s, sc_uint<8> X[4][4])
{
    printf("%s R\n", s);
    for (int i = 0; i < 4; i++)
    {
...
void Dump_rk(char *s, sc_uint<8> X[11][4][4])
{
    printf("%s rk\n", s);
    for (int i = 0; i < 11; i++)
    {
...
void AES_Base::AddRoundKey(sc_uint<8> a[4][4], const sc_uint<8> rk[4][4])
{
...
void AES_Base::SchedKey(sc_uint<8> k[4][4], sc_uint<8> W[11][4][4])
{
    sc_uint<3> i, j;
    sc_uint<8> tk[4][4];
    sc_uint<8> tt;
...
public:
    ...
    AES_Decrypt(sc_module_name name,
                ...
                sc_signal<sc_biguint<128>> &pOut_wire) : AES_Base(name)
    {
        CLK(pCLK);
        RST_X(pRST_X);
        In_req(pIn_req);
...
void AES_Decrypt::Reset(void)
{
    In_req = false;
    Out_ack = false;
}

void AES_Decrypt::Input(void)
{
    sc_uint<8> t;
...
void AES_Decrypt::MainThread(void)
{
    (void)Reset();
...
void AES_Decrypt::Substitution_Si(sc_uint<8> a[4][4])
{
    for (sc_uint<3> j = 0; j < 4; j++)
    {
        for (sc_uint<3> i = 0; i < 4; i++)
 ...
 */
void AES_Decrypt::ShiftRows_Right_Rotate(sc_uint<8> a[4][4])
{
    sc_uint<8> t;
...
void AES_Decrypt::InvMixColumns(sc_uint<8> a[4][4])
{
    sc_uint<8> t[4][4];
    ...
    for (sc_uint<3> j = 0; j < 4; j++)
    {
        for (sc_uint<3> i = 0; i < 4; i++)
        {
            t[i][j] = dmule(a[i][j]) ^ dmulb(a[(i + 1) & 3][j]) ^ dmuld(a[(i + 2) & 3][j]) ^ dmul9(a[(i + 3) & 3][j]);
        }
    }
...
void AES_Encrypt::Reset(void)
{
    In_req = false;
    Out_ack = false;
}

void AES_Encrypt::Input(void)
{
    sc_biguint<128> t_In_wire;
    sc_uint<8> t;
...
void AES_Encrypt::MainThread(void)
{
    (void)Reset();
...
void AES_Encrypt::Substitution_S(sc_uint<8> a[4][4])
{
    for (sc_uint<3> j = 0; j < 4; j++)
    {
        for (sc_uint<3> i = 0; i < 4; i++)
 ...
 * shifts[][][] array used at run time.
 */
void AES_Encrypt::ShiftRows_Left_Rotate(sc_uint<8> a[4][4])
{
    sc_uint<8> t;
...
void AES_Encrypt::MixColumns(sc_uint<8> a[4][4])
{
    sc_uint<8> t[4][4];
    ...
    for (sc_uint<3> j = 0; j < 4; j++)
    {
        for (sc_uint<3> i = 0; i < 4; i++)
        {
            t[i][j] = dmul2(a[i][j]) ^ dmul3(a[(i + 1) & 3][j]) ^ a[(i + 2) & 3][j] ^ a[(i + 3) & 3][j];
        }
    }
