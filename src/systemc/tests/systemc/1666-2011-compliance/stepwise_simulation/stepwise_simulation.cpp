...
void do_step(sc_time const &step)
{
    ...
    sc_assert(sc_delta_count() == delta + (!start_delta && step == SC_ZERO_TIME));
    start_delta = false;
}
...
int sc_main(int, char *[])
{
    ...
    {
        sc_assert(sc_pending_activity_at_current_time() || sc_time_to_pending_activity() > SC_ZERO_TIME);

        sc_assert(sc_pending_activity_at_future_time() || sc_time_to_pending_activity() == SC_ZERO_TIME);
