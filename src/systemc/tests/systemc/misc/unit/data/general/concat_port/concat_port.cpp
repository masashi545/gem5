...
void concat_port::entry()
{
    ...
    while (true)
    {
        ...
        case 0:
            c.write((tmp_a.range(7, 4), tmp_b.range(7, 4)));
            d.write((tmp_a, tmp_b));
            break;

        case 1:
            c.write((tmp_a.range(0, 7)));
            d.write((tmp_a.range(0, 7), tmp_b.range(0, 7)));
            break;

        case 2:
            c.write((tmp_a[0], tmp_b.range(1, 4), tmp_a.range(7, 5)));
            d.write((tmp_a.range(7, 4), tmp_b.range(7, 4),
                     tmp_a.range(3, 0), tmp_b.range(3, 0)));
            break;

        case 3:
            c.write(("1", tmp_b.range(2, 0), "0", tmp_a.range(2, 0)));
            d.write(("11", tmp_a.range(6, 0), tmp_b.range(6, 0)));
            break;

        default:
            cout << "Error: Mode " << mode.read()
                 << " does not exist!" << endl;
            break;
        }
        done.write(1);
