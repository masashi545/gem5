    ...
    void proc_tree(unsigned depth, unsigned width, bool as_method, bool spawn_only)
    {
        ...
        if (sc_time_stamp() == SC_ZERO_TIME || spawn_only)
            while (depth && w-- > 0)
            {
                ...
                {
                    sc_spawn(sc_bind(&top::proc_tree, this, depth - 1, width, !as_method, false), sc_gen_unique_name("thread"), &sp);
                }
                else // we are spawned as thread, spawn a method
                {
                    sp.spawn_method();
                    sc_spawn(sc_bind(&top::proc_tree, this, depth - 1, width, !as_method, false), sc_gen_unique_name("method"), &sp);
                }
            }
