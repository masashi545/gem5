...
SC_MODULE(X)
{
    ...
    void comb()
    {
        cout << "  X::comb() - " << sc_time_stamp() << "  " << tweak.read() << endl;
    }
    void sync()
    {
        for (;;)
        {
            wait();
            cout << "  X::sync() - " << sc_time_stamp() << "  " << tweak.read() << endl;
        }
    }
    sc_in_clk clk;
    sc_in<bool> tweak;
...
#define ACTION(action, descr)                                                                                   \
    {                                                                                                           \
        cout << descr << " - " << sc_time_stamp() << "   " << tweak.read() << endl;                             \
        action;                                                                                                 \
        cout << "                                        " << sc_time_stamp() << "   " << tweak.read() << endl; \
    }
