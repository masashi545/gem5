...
struct my_port : sc_in<bool>
{
    ...
    explicit my_port(const char *nm)
        : base_type(nm), direct_mod((std::string(nm) + "_direct_mod").c_str()), beoe_called(false) {}
...
SC_MODULE(sub_module)
{
    ...
    SC_CTOR(sub_module)
        : direct_port("direct_port"), delayed_port(0), beoe_called(false)
    {
    }
...
SC_MODULE(module)
{
    ...
    SC_CTOR(module)
        : direct_mod("direct_mod"), direct_port("direct_port"), delayed_mod(0), delayed_port(0), beoe_called(false)
    {
